...
import { deleteSoftware, getSoftwareList } from '#/api/cms/software';
import { TableAction } from '#/components/table/table-action';

import SoftwareForm from './form.vue';
...
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: SoftwareForm,
});
...
const gridOptions: VxeGridProps<SoftwareInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteSoftware({
    ids,
  });
  if (result.code === 0) {
